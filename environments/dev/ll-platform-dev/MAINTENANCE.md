...
1. Create or update the `ll-system-chat` Login item in the
   `legends-legacy-dev` 1Password vault with `name=ll-system-chat`,
   `namespace=ll`, and a generated password. Tag it with `sealedsecret` and
   `env:dev`, then generate the SealedSecret from the infrastructure repository:
   ...
   `templates/secrets/ll.ll-system-chat.sealed.yaml` file. The backend/worker and
   chat Pods consume its `password` key; its plaintext value is never stored in
   Git.
...
- Achievement announcements use the `password` key in the `ll-system-chat`
  Secret generated by the 1Password/SealedSecret workflow. Both the game
  backend/worker and chat Pods reference that same key; the plaintext value is
